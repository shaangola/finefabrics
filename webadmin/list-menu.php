...
$filter_caption=array("Menu Group Name","Order");
$sorting_array=array();
...
<?php include("header.php"); ?>
	  ...
	  <table width="100%" border="0" cellpadding="0" cellspacing="0">
        <tr>
		  ...
		  <table width="100%" border="0" class='mGrid'>
			...
			$sql="select mn.menu_id,mn.menu_name,mn.ispublish,mn.order_index,count(smn.menu_id) as child ".
			" from "._DB_PREFIX."menu as mn left join "._DB_PREFIX."menu as smn on mn.menu_id=smn.parent_id where mn.parent_id=0  $cond  group by mn.menu_id $orderby";
			...
			?>
					<tr class="<?php echo $classID;?>"  onclick="ChangeGridCSS(this,'<?php echo $classID; ?>')" onmouseover="mouseMove(this,'<?php echo $classID; ?>')" onmouseout="mouseOut(this,'<?php echo $classID; ?>')">
					  ...
					  <td valign="top"  nowrap="nowrap"><a href="list-submenu.php?parent_id=<?php echo $rsdata['menu_id'];?>">Menu Option</a></td>
					  ...
					  <?php if(isset($_SESSION["inte_sess_publish_employee"])){ ?>
						<span id="ststdl<?php echo $rsdata["menu_id"]; ?>">
							...
							<a href="manage-menu.php?mode=edit&id=<?php echo $rsdata["menu_id"].'&page='.$pageNum.''.$filterqstr.'&rpp='.$rpp;?>"><img src="images/modify.gif" border="0" style="cursor:pointer"></a>
							<?php }
							if(isset($_SESSION["inte_sess_delete_employee"])){ 
							if(intval($rsdata['child']) > 0){
								echo '<img src="images/locked_icon.gif" title="contain option">';
							}
							else{
							?>
						<?php echo '<a href="manage-menu.php?afid=&rpp='.$rpp.'&menu_id='.$rsdata["menu_id"].'&delmenu=delmenu&page='.$pageNum.''.$filterqstr.'" onClick="return cmd_delete(\''.$rsdata["menu_name"].'\');"><img src="images/delete_icon.png" border="0" title="Delete '.$rsdata["menu_name"].'"></a>';
								}	 ?>
							<?php }
							?>	
					  </td>
				 ...
				 <?php  $sno++;}
					  ...
					  <tr><td valign="middle" align="center" colspan="5">No Record Found !</td></tr>
					<?php } ?>
          </table>
		  </div>
		  </td>
	...
	<a style="cursor:pointer" href="manage-menu.php?mode=add<?php echo $filterqstr.'&page='.$pageNum.'&rpp='.$rpp; ?>" class="graybgbtn">Create New</a>
		<?php } ?>
				</td>
