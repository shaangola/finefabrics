...
$request_array=array("menu_id" =>"","menu_name" =>"","ispublish" =>"","order_index" =>"","icon" =>"","is_dashboard_icon" =>"","parent_id"=>"","page_name"=>"");
$extraurl_array=array("rpp" =>"","page" =>"","fcondvalue" =>"","fcond" =>"","ord" =>"","srt" =>"","parent_id" => "");
$extraurl='';
$adminmsg='';
...
if(isset($_REQUEST["meng_menu"]) && $_REQUEST["meng_menu"]=="add"){ 
	...
	$sql="insert into "._DB_PREFIX."menu(menu_name,parent_id,page_name,ispublish,order_index,icon,is_dashboard_icon)" .
	" values('".$request_array["menu_name"]."','".$request_array["parent_id"]."','".$request_array["page_name"]."','".$ispublish."','".$request_array["order_index"]."','".$request_array["icon"]."','".$is_dashboard_icon."')";
	$rowafftected=$db->query($sql);
	
	if($rowafftected > 0){
		$_SESSION["inte_sess_ackmsg"]='<div id="ack_div" class="error_message truegreen"> Menu option added successfully</div>';
	}
	else{
		$_SESSION["inte_sess_ackmsg"]='<div id="ack_div" class="error_message errorred"> Menu option Insertion Failed!</div>';
	}

	
	echo "<script>window.location.href='list-submenu.php?opt=done$extraurl';</script>";
	exit;
}
	...
	$sql=" update "._DB_PREFIX."menu set menu_name='".$request_array['menu_name']."',page_name='".$request_array['page_name']."',parent_id='".$request_array['parent_id']."',is_dashboard_icon='".$is_dashboard_icon."',ispublish='".$ispublish."',order_index='".$request_array['order_index']."',icon='".$request_array['icon']."' where menu_id='".$request_array['menu_id']."'";
	$rowAffected=$db->query($sql);
	
	if($rowAffected > 0){ 	$_SESSION["inte_sess_ackmsg"]='<div id="ack_div" class="error_message truegreen"> Menu  option Updated successfully</div>';}
	else{ $_SESSION["inte_sess_ackmsg"]='<div id="ack_div" class="error_message errorred"> Menu option Updation Failed!</div>';}
	...
	$rowafftected=$db->query($sql);
	if($rowafftected > 0){
		$_SESSION["inte_sess_ackmsg"]='<div id="ack_div" class="error_message truegreen"> Menu option Deleted Successfully</div>';
	}
	else{
		$_SESSION["inte_sess_ackmsg"]='<div id="ack_div" class="error_message errorred"> Menu option Deletetion Failed!</div>';
	}
...
if($mode=="edit"){
	...
	$request_array['page_name']=stripslashes($data["page_name"]);
	...
	$contentBox='Update Menu Option';

}
else{
	if(!isset($_SESSION["inte_sess_create_employee"])){
		$_SESSION["inte_sess_privmsg"]='<div id="ack_div" class="error_message errorred">Access Out of Privilege</div>';
		header("Location: home.php");
			exit;
	}
        ...
        <tr>
		 ...
		 <form method="post" enctype="multipart/form-data" name="frmSearch" id="frmSearch">
		  ...
		  <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table1">
			...
			?>
			<tr>
				<td width="151" valign="top"  style="padding:5px;"><strong>Menu Option Name *</strong></td>
			...
			<tr>
				<td width="151" valign="top"  style="padding:5px;"><strong>Page Name</strong></td>
				<td width="198" valign="top"  style="padding:5px;"><input  type="text" onkeypress="return hideLevel('page_name');"   name="page_name" id="page_name" value="<?php echo $request_array['page_name'];?>" class="formfld" style="width:150px;"/><label class="error" id="lbl_page_name"></label></td>
				<td  style="padding:5px;"></td>
				
				<td width="117" valign="top"  style="padding:5px;"><strong>Icon</strong> </td>
				<td width="610" valign="top"  style="padding:5px;"><input  type="text" onkeypress="return hideLevel('icon');"   name="icon" id="icon" value="<?php echo $request_array['icon'];?>" class="formfld" style="width:150px;"/><label class="error" id="lbl_icon"></label></td>	
					</tr>
			 ...
			 <tr>
				...
				<input type="hidden" name="meng_menu" value="<?php echo $mode; ?>" id="meng_menu" />
				...
				<input type="hidden"  name="parent_id" id="parent_id" value="<?php echo $extraurl_array["parent_id"];?>" />
...
function validateForm(){
	...
	var theCaption=new Array("menu option name","order index");
