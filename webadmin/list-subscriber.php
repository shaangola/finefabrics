...
$filter_value=array("s.subscriber_name","s.subscriber_email","s.occupied","s.occupied_date","s.subscribedatetime");
$filter_caption=array("Name","Email ID","Avail","Avail Date","Subscribe Date");
$sorting_array=array();
	  ...
	  <table width="100%" border="0" cellpadding="0" cellspacing="0">
        <tr>
		  ...
		  <table width="100%" border="0" class='mGrid'>
            ...
            <?php 
			 $sql="select s.id,s.subscriber_name,s.subscriber_email,s.ispublish,date(s.subscribedatetime) as subscribedatetime,s.occupied_date,s.occupied, ".
			 " DATE_ADD(date(subscribedatetime), INTERVAL validity DAY) as end_date from "._DB_PREFIX."subscription as s   ".
			 " where DATE_ADD(date(subscribedatetime), INTERVAL validity DAY) >=current_date and s.ispublish=1  $cond $orderby";
			...
			?>
					<tr class="<?php echo $classID;?>"  onclick="ChangeGridCSS(this,'<?php echo $classID; ?>')" onmouseover="mouseMove(this,'<?php echo $classID; ?>')" onmouseout="mouseOut(this,'<?php echo $classID; ?>')">
					  ...
					  <td class='ob_gC1' id="occupied<?php echo $rsdata["id"]; ?>">
					  	<?php 
						echo stripcslashes($rsdata['occupied']); 
						if($rsdata['occupied']=="NO"){ ?>
                        <img src="images/blocked.gif" border="0" style="cursor:pointer;"  onClick="getAjxRespMD('occupied<?php echo $rsdata["id"]; ?>','occupied_date<?php echo $rsdata["id"]; ?>','AjaxHandler.php?query=avlsubs&id=<?php echo $rsdata["id"]; ?>')"  title="Click here to avail" />
                        
						<?php }
						?>
                      </td>
					  <td class='ob_gC1' id="occupied_date<?php echo $rsdata["id"]; ?>">
					  	<?php echo stripcslashes($rsdata['occupied_date']); ?>
                      </td>
					 <td class='ob_gC1'><?php echo stripcslashes($rsdata['subscribedatetime']); ?></td>
					  <td class='ob_gC1'><?php echo stripcslashes($rsdata['end_date']); ?></td>
					  ...
					  <?php if(isset($_SESSION["inte_sess_publish_subscriber"])){ ?>
						<span id="ststdl<?php echo $rsdata["id"]; ?>">
							...
							if(isset($_SESSION["inte_sess_edit_subscriber"])){ ?>
								<a href="manage-subscriber.php?mode=edit&id=<?php echo $rsdata["id"].'&page='.$pageNum.''.$extra_url_filter.''.$extra_url_sorting.'&rpp='.$rpp;?>"><img src="images/modify.gif" border="0" style="cursor:pointer" title="Edit <?php echo $rsdata["subscriber_name"];?>"></a>
						<?php	}
							if(isset($_SESSION["inte_sess_delete_subscriber"])){ 
								
									echo '<a href="manage-subscriber.php?afid='.session_id().'&rpp='.$rpp.'&id='.$rsdata["id"].'&delsubscription=delsubscription&page='.$pageNum.''.$extra_url_filter.''.$extra_url_sorting.'" onClick="return cmd_delete(\''.$rsdata["subscriber_name"].'\');"><img src="images/delete_icon.png" border="0" title="Delete '.$rsdata["subscriber_name"].'"></a>';
