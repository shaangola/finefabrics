...
if(!isset($_SESSION["inte_sess_view_newsletter"])){
...
$filter_value=array("s.newsletter_title","s.createddate");
$filter_caption=array("Title","Created Date");
$sorting_array=array();
...
$filterqstr1="";
$orderby="order by s.newsletter_id desc ";
	  ...
	  <table width="100%" border="0" cellpadding="0" cellspacing="0">
        <tr>
		  ...
		  <table width="100%" border="0" class='mGrid'>
            ...
            <?php 
			 $sql="select s.newsletter_id,s.newsletter_title,s.newsletter_id,s.ispublish,s.createddate from "._DB_PREFIX."newsletter as s   where 1  $cond $orderby";
			...
			?>
					<tr class="<?php echo $classID;?>"  onclick="ChangeGridCSS(this,'<?php echo $classID; ?>')" onmouseover="mouseMove(this,'<?php echo $classID; ?>')" onmouseout="mouseOut(this,'<?php echo $classID; ?>')">
					  ...
					  <td class='ob_gC1'><?php echo stripcslashes($rsdata['newsletter_title']); ?></td>
					   <td class='ob_gC1'><?php echo stripcslashes($rsdata['createddate']); ?></td>
					 
					  <td class='ob_gC1'>
					  <?php if(isset($_SESSION["inte_sess_publish_newsletter"])){ ?>
						<span id="ststdl<?php echo $rsdata["newsletter_id"]; ?>">
							<?php if($rsdata["ispublish"]=='1'){ ?>
							<img src="images/publish.jpg" border="0" style="cursor:pointer;" title="Change Status"  onClick="getAjxResp('ststdl<?php echo $rsdata["newsletter_id"]; ?>','AjaxHandler.php?query=changestatus&field=ispublish&tdid=ststdl<?php echo $rsdata["newsletter_id"]; ?>&status=0&tname=<?php echo _DB_PREFIX; ?>newsletter&ccol=newsletter_id&cval=<?php echo $rsdata["newsletter_id"]; ?>')"  />
									
							<?php }	else{ ?>
							<img src="images/blocked.gif" border="0" style="cursor:pointer;"  onClick="getAjxResp('ststdl<?php echo $rsdata["newsletter_id"]; ?>','AjaxHandler.php?query=changestatus&field=ispublish&tdid=ststdl<?php echo $rsdata["newsletter_id"]; ?>&status=1&tname=<?php echo _DB_PREFIX; ?>newsletter&ccol=newsletter_id&cval=<?php echo $rsdata["newsletter_id"]; ?>')"  title="Change Status" />
							<?php	}  	?>
							</span>
							<?php }
							if(isset($_SESSION["inte_sess_edit_newsletter"])){ ?>
							<a href="manage-newsletter.php?mode=edit&id=<?php echo $rsdata["newsletter_id"].'&page='.$pageNum.''.$extra_url_filter.''.$extra_url_sorting.'&rpp='.$rpp;?>"><img src="images/modify.gif" border="0" style="cursor:pointer" title="Delete <?php echo $rsdata["newsletter_title"];?>"></a>
							<?php }
							if(isset($_SESSION["inte_sess_delete_newsletter"])){ 
								if($db1->isFound(_DB_PREFIX."subcategory","newsletter_id='".$rsdata["newsletter_id"]."'")){ 
									echo '<img src="images/locked_icon.gif" title="Contain Subcategory" alt="Contain Subcategory" />';
								}
								else{
									echo '<a href="manage-newsletter.php?afid='.session_id().'&rpp='.$rpp.'&newsletter_id='.$rsdata["newsletter_id"].'&delnewsletter=delnewsletter&page='.$pageNum.''.$extra_url_filter.''.$extra_url_sorting.'" onClick="return cmd_delete(\''.$rsdata["newsletter_title"].'\');"><img src="images/delete_icon.png" border="0" title="Delete '.$rsdata["newsletter_title"].'"></a>';
				 ...
				 <?php  $sno++;}
					  ...
					  <tr><td align="center" valign="middle" colspan="6">No Record Found !</td></tr>
					<?php } ?>
          </table>
		  </div>
		  </td>
     ...
     <tr bgcolor="#ffffff">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			?>	
				...
				<?php if(isset($_SESSION["inte_sess_create_newsletter"])){ ?>
	<a style="cursor:pointer" href="manage-newsletter.php?mode=add<?php echo $filterqstr.'&page='.$pageNum.'&rpp='.$rpp; ?>" class="graybgbtn">Create New</a>
		<?php } ?>
				</td>
				<td width="15%" align="left" valign="middle"><?php echo 'Records : '.$pageFrom.' to '.($rocordperpage+$offset).' of '.$numrows;?> </td>	
					...
					<?php if(isset($_SESSION["inte_sess_filter_newsletter"])){ ?>
